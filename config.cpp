...
Config::Config()//为所有配置参数设置默认值，即使用户不传入命令行参数，服务器也能以默认配置启动。
{
    ...
    LOGWrite=0;
     ...
     //推理引擎选择
     engine_type = const_cast<char *>("mock");
     model_path = const_cast<char *>("");

     //推理调度配置
     batch_window_ms = 10;
     max_batch_size = 8;
     max_concurrent_batches = 2;
     max_queue_size = 0;  // 0 = 无限制
...
{
    int opt;
    const char *str = "p:l:m:o:s:t:c:a:b:n:T:C:Q:E:M:";
    ...
    while ((opt = getopt(argc, argv, str)) != -1) {
        ...
        case 'C': max_concurrent_batches = atoi(optarg); break;
        case 'Q': max_queue_size = atoi(optarg); break;
        case 'E': engine_type = optarg; break;
